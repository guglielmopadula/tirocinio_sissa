...
namespace Step85
{
  ...
  template <int dim>
    class SphereCustom : public Function<dim>
    {
    public:
      /**
       * Constructor, takes the center and radius of the sphereCustom.
       */
      SphereCustom(const Point<dim> &center = Point<dim>(), const double radius = 1): radius(radius), center(center){}

      double
      value(const Point<dim> & point,
            const unsigned int component = 0) const override
      {
        return -center.distance_square(point)+radius*radius;
      }
