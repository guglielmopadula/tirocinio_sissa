...
points,points_zero,points_old,newmesh_indices_local,triangles,newtriangles_zero,newtriangles_local_1,newtriangles_local_2,newtriangles_local_3,edge_matrix=getinfo("hull_negative.stl",True)
...
param_ffd=np.zeros([500,45])

for i in range(0,500):
    if i%100==0:
        print(i)
    param= (np.random.uniform(0, 1, 45))
    param[0:27]=-0.15*param[0:27]
    ffd.reset_weights()
    ...
    alls[i]=temp1_new
    meshio.write_points_cells("hull_negative_{}.stl".format(i), points_new, [("triangle", triangles)])
