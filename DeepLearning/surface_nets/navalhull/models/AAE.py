...
class AAE(LightningModule):
    ...
    def __init__(self,data_shape,reduced_data_shape,temp_zero,local_indices_1,local_indices_2,newtriangles_zero,pca,edge_matrix,vertices_face_x,vertices_face_xy,k,latent_dim,batch_size,drop_prob,ae_hyp=0.999,hidden_dim: int= 500,**kwargs):
        super().__init__()
    ...
    def configure_optimizers(self):
        optimizer_ae = torch.optim.AdamW(itertools.chain(self.encoder.parameters(), self.decoder.parameters()), lr=0.00009)
        optimizer_disc = torch.optim.AdamW(self.discriminator.parameters(), lr=2e-4)
