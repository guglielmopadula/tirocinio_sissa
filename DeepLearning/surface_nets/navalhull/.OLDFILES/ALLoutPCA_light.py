...
class Data(LightningDataModule):
    ...
    def __init__(
        ...
        self.reduced=self.pca.transform(self.data.reshape(self.num_samples,-1))
        self.reduced_train=self.pca.transform(self.data_train[:].reshape(self.data_train[:].shape[0],-1))
        self.reduced_val=self.pca.transform(self.data_val[:].reshape(self.data_val[:].shape[0],-1))
        self.reduced_test=self.pca.transform(self.data_test[:].reshape(self.data_test[:].shape[0],-1))
...
class VAE(LightningModule):
    ...
    def __init__(self,data_shape,temp_zero,local_indices,M1,M2,M3,pca,hidden_dim: int= 300,latent_dim: int = LATENT_DIM,lr: float = 0.0002,b1: float = 0.5,b2: float = 0.999,batch_size: int = BATCH_SIZE,**kwargs):
        ...
        self.vol_norm=VolumeNormalizer(self.temp_zero, self.M1, self.M2, self.M3, self.local_indices)
    ...
    def sample_mesh(self,mean=None,var=None):
        ...
        temp=self.decoder(z).reshape(1,-1)
        temp=self.pca.inverse_transform(temp)
        temp=temp.reshape(1,-1,3)
        temp=self.vol_norm(temp)
        temp=temp.reshape(1,-1,3)
        return temp
...
class BEGAN(LightningModule):
    ...
    def sample_mesh(self):
        ...
        temp=self.generator(z).reshape(1,-1)
        temp=self.pca.inverse_transform(temp)
        temp=temp.reshape(1,-1,3)
        temp=self.vol_norm(temp)
        temp=temp.reshape(1,-1,3)
...
for wrapper, name in d.items():
    ...
    meshio.write_points_cells('test_'+name+'_extPCA.stl',oldmesh,[("triangle", data.oldM)])
    error=0
    ...
    if name!="GAN" and name!="BEGAN":
        error=0
        latent=model.get_latent(data.reduced[:])
