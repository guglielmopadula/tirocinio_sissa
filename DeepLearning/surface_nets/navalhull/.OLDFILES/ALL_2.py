...
NUMBER_SAMPLES=600
STRING="/home/cyberguli/tirocinio_sissa/DeepLearning/surface_nets/navalhull/hull_test_{}.stl"
AVAIL_GPUS = torch.cuda.device_count()
BATCH_SIZE = NUMBER_SAMPLES//30
...
def multi_cubic(a, b, c, d):
    ...
    temp2=((p<0)& ((4*p**3+27*q**2)>0)).int()*(-2*torch.abs(q)/q)*torch.sqrt(torch.abs(p)/3)*torch.cosh(1/3*myacosh(3*torch.abs(q)/(2*torch.abs(p))*torch.sqrt(3/torch.abs(p))))
    temp3=((p<0)& ((4*p**3+27*q**2)<0)).int()*2*mysqrt(torch.abs(p)/3)*torch.max(torch.stack((torch.cos(1/3*myacos(3*q/(2*p)*torch.sqrt(3/torch.abs(p)))-2*torch.pi*0/3),torch.cos(1/3*myacos(3*q/(2*p)*torch.sqrt(3/torch.abs(p)))-2*torch.pi*1/3),torch.cos(1/3*myacos(3*q/(2*p)*torch.sqrt(3/torch.abs(p)))-2*torch.pi*2/3))))
    return temp1+temp2+temp3-b/(3*a)
    
def getinfo(stl,flag):
    mesh=meshio.read(stl)
    points_old=torch.tensor(mesh.points.astype(np.float32))
    points=points_old[((points_old[:,2]>0) & (points_old[:,0]>0) & (points_old[:,1]>0))]
    points_zero=points_old[((points_old[:,2]>=0) & (points_old[:,0]>=0) & (points_old[:,1]>=0))]
    if flag==True:
        newmesh_indices_global=np.arange(len(mesh.points))[((points_old[:,2]>0) & (points_old[:,0]>0) & (points_old[:,1]>0))].tolist()
        ...
        newmesh_indices_global_zero=np.arange(len(mesh.points))[((points_old[:,2]>=0) & (points_old[:,0]>=0) & (points_old[:,1]>=0))].tolist()
        newtriangles_zero=[]
        for T in triangles:
        ...
        newmesh_indices_local=np.arange(len(points_zero))[((points_zero[:,2]>0) & (points_zero[:,0]>0) & (points_zero[:,1]>0))].tolist()
    ...
    return points,points_zero,points_old,newmesh_indices_local,triangles,newtriangles_zero,newtriangles_local_1,newtriangles_local_2,newtriangles_local_3,edge_matrix,vertices_face,[cvxpylayer,cvxpylayer_single]
...
def volume_norm(points,points_zero,newtriangles_zero, vertices_face,cvxpylayer):
    ...
    indices=((points_zero_2[:,:,2]>0) & (points_zero_2[:,:,0]>0) & (points_zero_2[:,:,1]>0))
    points_zero_2[indices]=points.reshape(-1,3)
...
def volume_norm_single(points,points_zero,newtriangles_zero, vertices_face,cvxpylayer):
    volume_const=volume_2_x(points_zero[newtriangles_zero].unsqueeze(0))
    points_zero_2=points_zero.clone().unsqueeze(0).repeat(len(points),1,1)
    indices=((points_zero_2[:,:,2]>0) & (points_zero_2[:,:,0]>0) & (points_zero_2[:,:,1]>0))
    points_zero_2[indices]=points.reshape(-1,3)
...
class Data(LightningDataModule):
    def get_size(self):
        temp,_,_,_,_,_,_,_,_,_,_,_=getinfo(STRING.format(0),False)
    ...
    def __init__(
        ...
        self.num_samples=num_samples
        _,self.temp_zero,self.oldmesh,self.local_indices,self.oldM,self.newtriangles_zero,_,_,_,self.edge_matrix,self.vertices_face,self.cvxpylayer=getinfo(STRING.format(0),True)
        vertices_face_2=copy.deepcopy(self.vertices_face)
        self.vertices_face=torch.zeros(self.get_size()[1],len(self.newtriangles_zero))
        ...
        for i in range(0,self.num_samples):
            if i%100==0:
                print(i)
            self.data[i],_,_,_,_,_,_,_,_,_,_,_=getinfo(STRING.format(i),False)
        # Assign train/val datasets for use in dataloaders
        self.pca=PCA(REDUCED_DIMENSION)
        self.pca.fit(self.data.reshape(self.num_samples,-1))
        self.data_train, self.data_val,self.data_test = random_split(self.data, [math.floor((1/3)*self.num_samples), math.floor((1/3)*self.num_samples),self.num_samples-math.floor((1/3)*self.num_samples)-math.floor(1/3*self.num_samples)])    
...
class VolumeNormalizer(nn.Module):
    def __init__(self,temp_zero,newtriangles_zero,vertices_face,cvxpylayer):
        super().__init__()
    ...
    def forward(self, x):
        if self.flag:
            return volume_norm(x, self.temp_zero, self.newtriangles_zero, self.vertices_face, self.cvxpylayer[0])
        else:
            return volume_norm_single(x, self.temp_zero, self.newtriangles_zero, self.vertices_face, self.cvxpylayer[1])
...
class Smoother(nn.Module):
    ...
    def forward(self,x):
        ...
        temp[((temp[:,:,2]>0) & (temp[:,:,0]>0) & (temp[:,:,1]>0))]=x.reshape(-1,3)
...
class Decoder_base(nn.Module):
    def __init__(self, latent_dim, hidden_dim, data_shape,temp_zero,newtriangles_zero,pca,edge_matrix,vertices_face,cvxpylayer,k):
        ...
        self.fc4 = LBR(hidden_dim, int(np.prod(self.data_shape)))
        self.fc5 = LBR(int(np.prod(self.data_shape)), int(np.prod(self.data_shape)))
        self.fc6 = LBR(int(np.prod(self.data_shape)), int(np.prod(self.data_shape)))
        self.fc7 = nn.Linear(int(np.prod(self.data_shape)), int(np.prod(self.data_shape)))
        self.fc8=Smoother(self.edge_matrix, self.k,self.temp_zero)
        self.fc9=VolumeNormalizer(self.temp_zero,self.newtriangles_zero,self.vertices_face,self.cvxpylayer)
        self.relu = nn.ReLU()
...
class Encoder_base(nn.Module):
    def __init__(self, latent_dim, hidden_dim,data_shape,pca):
        ...
        self.pca=pca
        self.fc1 = LBR(int(np.prod(self.data_shape)), int(np.prod(self.data_shape)))
        self.fc2 = LBR(int(np.prod(self.data_shape)), int(np.prod(self.data_shape)))
        self.fc3 = LBR(int(np.prod(self.data_shape)), int(np.prod(self.data_shape)))
        self.fc4 = LBR(int(np.prod(self.data_shape)), hidden_dim)
        self.fc5 = LBR(hidden_dim, hidden_dim)
        self.fc6 = LBR(hidden_dim, hidden_dim)
        self.fc7 = nn.Linear(hidden_dim, latent_dim)
        self.tanh=nn.Tanh()
    ...
    def forward(self, x):
        ...
        hidden=self.fc1(x)
        mu=self.fc7(self.fc6(self.fc5(self.fc4(self.fc3(self.fc2(self.fc1(hidden)))))))
        mu=self.batch_mu(mu)
        return mu
...
class VAE(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # kl
        kl_val = kl_divergence(z_sampled, mu, sigma)

        # elbo
        elbo = (kl_val - recon_loss)
...
for i in range(NUMBER_SAMPLES):
    temp_zero=data.temp_zero.clone().numpy()
    temp_zero[(temp_zero[:,2]>0)*(temp_zero[:,0]>0)*(temp_zero[:,1]>0)]=data.data[i].reshape(data.get_size()[1],data.get_size()[2]).detach().numpy()
...
for wrapper, name in d.items():
    ...
    model=wrapper(data.get_reduced_size(),data.temp_zero,data.newtriangles_zero,data.pca,data.edge_matrix,data.vertices_face,data.cvxpylayer)
    ...
    if hasattr(model, 'decoder'):
        model.decoder.decoder_base.fc9.flag=False
    else:
        model.generator.decoder_base.fc9.flag=False
    ...
    #curvature_mean_sampled=np.zeros([NUMBER_SAMPLES,np.max(data.newtriangles_zero)+1])
    ...
    for i in range(NUMBER_SAMPLES):
        ...
        temparr[i]=temp
        oldmesh[(oldmesh[:,2]>0)*(oldmesh[:,0]>0)*(oldmesh[:,1]>0)]=temp.reshape(-1,3)
        ...
        temp_zero[(temp_zero[:,2]>0)*(temp_zero[:,0]>0)*(temp_zero[:,1]>0)]=temp.reshape(data.get_size()[1],data.get_size()[2]).numpy()
        mesh_object=trimesh.base.Trimesh(temp_zero,data.newtriangles_zero,process=False)
...
for i in range(NUMBER_SAMPLES):
    ...
    temparr[i]=temp[((temp[:,2]>0) & (temp[:,0]>0) & (temp[:,1]>0))]
