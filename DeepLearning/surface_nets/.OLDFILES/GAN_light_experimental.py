...
def getinfo(stl,flag):
    ...
    if flag==True:
        ...
        triangles=0
        newtriangles=0
    return points,points_old,triangles,newtriangles
...
points,_,_,newtriangles=getinfo(STRING.format(0),True)
volume_const=volume(points[newtriangles])**(1/3)

class Data(LightningDataModule):
    def get_size(self):
        temp,self.oldmesh,self.oldM,self.M=getinfo(STRING.format(0),True)
    ...
    def setup(self, stage=None):
        if stage=="fit":
            ...
            for i in range(0,self.num_samples):
                if i%100==0:
                    print(i)
                self.data[i],_,_,_=getinfo(STRING.format(i),False)
...
class GAN(LightningModule):
    ...
    def __init__(self,data_shape,M,hidden_dim: int= 400,latent_dim: int = 8,lr: float = 0.0002,b1: float = 0.5,b2: float = 0.999,batch_size: int = BATCH_SIZE,**kwargs):
        super().__init__()
