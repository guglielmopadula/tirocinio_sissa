...
points,points_zero,points_old,newmesh_indices_local,triangles,newtriangles_zero,newtriangles_local_1,newtriangles_local_2,newtriangles_local_3,edge_matrix=getinfo("hullpreprocessed.stl",True)
...
param_vec=np.zeros([150,8])            

t=[0,1]
temp1_new=temp1.copy()
...
for counter in range(150):
    ...
    a=scipy.stats.truncnorm.rvs(0.05,0.20,size=3)
    c=scipy.stats.truncnorm.rvs(0.05,0.20,size=3)
    param_vec[counter,0]=alpha
    param_vec[counter,1]=beta
    param_vec[counter,2:5]=a
    param_vec[counter,5:]=c
    v=vec_def(alpha,beta,a,c)
    j=jac_def(alpha,beta,a,c)
    for i in range(len(temp1)):
        temp1_new[i]=solve_ivp(v,t,temp1[i].astype(np.single),dense_output=True,jac=j,method="BDF").sol(0.01)
    temp_new=temp.copy()
...
for j in range(150,300):
    ...
    for i in range(len(temp1)):
        temp1_new[i]=solve_ivp(v_vec,t,temp1[i].astype(np.single),dense_output=True,jac=jac_vec,method="BDF").sol(1)
    temp_new=temp.copy()
    points_new=points_old.copy()
    a1=np.random.uniform(0.8,1.2,1)
    b1=np.random.uniform(0.8,1.2,1)
...
param_ffd=np.zeros([200,47])

for i in range(300,500):
    if i%100==0:
        print(i)
    param= (np.random.uniform(0, 1, 47))
    ffd.reset_weights()
    ffd.array_mu_x[0, 2, 1] += -0.1*param[0]
    ffd.array_mu_x[0, 2, 0] += -0.1*param[1]
    ffd.array_mu_x[0, 1, 1] += -0.1*param[2]
    ffd.array_mu_x[0, 1, 0] += -0.1*param[3]
    ffd.array_mu_x[0, 0, 1] += -0.1*param[4]
    ffd.array_mu_x[0, 4, 2] += -0.1*param[7]
    ffd.array_mu_x[0, 5, 2] += -0.1*param[8]
    ffd.array_mu_x[0, 4, 3] += -0.1*param[9]
    ffd.array_mu_x[0, 5, 3] += -0.1*param[10]
    ffd.array_mu_x[0, 4, 0] += -0.1*param[11]
    ffd.array_mu_x[0, 4, 1] += -0.1*param[12]
    ffd.array_mu_x[0, 5, 0] += -0.1*param[13]
    ffd.array_mu_x[0, 5, 1] += -0.1*param[14]
    ffd.array_mu_x[0, 2, 2] += -0.1*param[15]
    ffd.array_mu_x[0, 3, 3] += -0.1*param[16]
    ffd.array_mu_x[0, 2, 3] += -0.1*param[17]
    ffd.array_mu_x[0, 3, 2] += -0.1*param[18]
    ffd.array_mu_x[0, 2, 0] += -0.1*param[19]
    ffd.array_mu_x[0, 2, 1] += -0.1*param[20]
    ffd.array_mu_x[0, 3, 0] += -0.1*param[21]
    ffd.array_mu_x[0, 3, 1] += -0.1*param[22]
    ffd.array_mu_x[0, 0, 2] += -0.1*param[23]
    ffd.array_mu_x[0, 1, 2] += -0.1*param[24]
    ffd.array_mu_x[0, 0, 3] += -0.1*param[25]
    ffd.array_mu_x[0, 1, 3] += -0.1*param[26]
    ffd.array_mu_z[1, 2, 1] += param[5]
    ffd.array_mu_z[2, 1, 2] += param[6]
    ffd.array_mu_z[1, 2, 2] += param[27]
    ffd.array_mu_z[1, 2, 3] += param[28]
    ffd.array_mu_z[1, 4, 3] += param[29]
    ffd.array_mu_z[1, 5, 3] += param[30]
    ffd.array_mu_z[2, 2, 4] += param[31]
    ffd.array_mu_z[2, 2, 5] += param[32]
    ffd.array_mu_z[2, 3, 4] += param[33]
    ffd.array_mu_z[2, 3, 5] += param[34]
    ffd.array_mu_y[1, 2, 2] += param[35]
    ffd.array_mu_y[1, 2, 3] += param[36]
    ffd.array_mu_y[1, 4, 3] += param[37]
    ffd.array_mu_y[1, 5, 3] += param[38]
    ffd.array_mu_y[2, 2, 4] += param[39]
    ffd.array_mu_y[2, 2, 5] += param[40]
    ffd.array_mu_y[2, 3, 4] += param[41]
    ffd.array_mu_y[2, 3, 5] += param[42]

    
    a1=np.random.uniform(0.75,1.25,1)
...
print("Vector Variance is", np.sum(np.var(alls[:150],axis=0)))
print("Laplacian Variance is", np.sum(np.var(alls[150:300],axis=0)))
print("FFD variance is",np.sum(np.var(alls[300:],axis=0)))

ffd_dict={"params":param_ffd,"output":alls[300:]}
vec_dict={"params":param_vec,"output":alls[0:150]}
lapl_dict={"params":param_lapl,"output":alls[150:300]}
