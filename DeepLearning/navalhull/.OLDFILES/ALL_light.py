...
class VAE(LightningModule):
    ...
    def __init__(self,data_shape,temp_zero,local_indices,M1,M2,M3,hidden_dim: int= 300,latent_dim: int = LATENT_DIM,lr: float = 0.0002,b1: float = 0.5,b2: float = 0.999,batch_size: int = BATCH_SIZE,**kwargs):
        super().__init__()
...
class BEGAN(LightningModule):
    ...
    class Discriminator(nn.Module):
        def __init__(self, latent_dim, hidden_dim,data_shape,temp_zero, M1, M2, M3, local_indices):
            ...
            self.decoder=Decoder_base(latent_dim, hidden_dim, data_shape, temp_zero, M1, M2, M3, local_indices)
    ...
    def __init__(self,data_shape,temp_zero,local_indices,M1,M2,M3,hidden_dim: int= 300,latent_dim: int = LATENT_DIM,lr: float = 0.0002,b1: float = 0.5,b2: float = 0.999,batch_size: int = BATCH_SIZE,**kwargs):
        ...
        # networks
        self.data_shape = data_shape
        self.discriminator = self.Discriminator(latent_dim=self.hparams.latent_dim,hidden_dim=self.hparams.hidden_dim ,data_shape=self.data_shape,M1=self.M1,M2=self.M2,M3=self.M3,local_indices=self.local_indices,temp_zero=self.temp_zero)
        self.generator = self.Generator(data_shape=self.data_shape, latent_dim=self.latent_dim ,hidden_dim=self.hparams.hidden_dim,M1=self.M1,M2=self.M2,M3=self.M3,local_indices=self.local_indices,temp_zero=self.temp_zero)
...
class GAN(LightningModule):
    ...
    def __init__(self,data_shape,temp_zero,local_indices,M1,M2,M3,hidden_dim: int= 400,latent_dim: int = LATENT_DIM,lr: float = 0.0002,b1: float = 0.5,b2: float = 0.999,batch_size: int = BATCH_SIZE,**kwargs):
        super().__init__()
        self.save_hyperparameters()
        self.M1=M1
        self.M2=M2
        ...
        # networks
        self.data_shape = data_shape
        self.generator = self.Generator(latent_dim=self.hparams.latent_dim,hidden_dim=self.hparams.hidden_dim ,data_shape=self.data_shape,M1=self.M1,M2=self.M2,M3=self.M3,local_indices=self.local_indices,temp_zero=self.temp_zero)
...
class AAE(LightningModule):
    ...
    def __init__(self,data_shape,temp_zero,local_indices,M1,M2,M3,hidden_dim: int= 300,latent_dim: int = LATENT_DIM,lr: float = 0.0002,b1: float = 0.5,b2: float = 0.999,batch_size: int = BATCH_SIZE, ae_hyp=0.999,**kwargs):
        super().__init__()
        self.save_hyperparameters()
        self.M1=M1
        self.M2=M2
...
class VAEGAN(LightningModule):
    ...
    def __init__(self,data_shape,temp_zero,local_indices,M1,M2,M3,hidden_dim: int= 300,latent_dim: int = 1,lr: float = 0.0002,b1: float = 0.5,b2: float = 0.999,batch_size: int = BATCH_SIZE, ae_hyp=0.999,**kwargs):
        super().__init__()
        self.save_hyperparameters()
        self.M1=M1
        self.M2=M2
...
for wrapper, name in d.items():
    ...
    model=wrapper(data.get_size(),data.temp_zero,data.local_indices,data.M1,data.M2,data.M3)
    #LITTLE HACK
    ...
    meshio.write_points_cells('test_'+name+'.stl',oldmesh,[("triangle", data.oldM)])
    error=0
    ...
    for i in range(NUMBER_SAMPLES):
        ...
        oldmesh[oldmesh[:,2]>0]=temp.reshape(model.data_shape[1],model.data_shape[2]).detach()
