...
def get_map_reverse(filename):
    Y,X,F=readtet6(filename)
    t = sympy.symbols("t",real=True)
    denom=0
    for i in range(len(F)):
        matrixX=get_matrix(X,F[i])
...
fun_a,symb_a,F_a=get_map_reverse("morph_a.tet6") 
pfun_a=sympy.lambdify(symb_a,fun_a)
print("Sympyng second map")
fun_b,symb_b,F_b=get_map_forward("morph_b.tet6") 
pfun_b=sympy.lambdify(symb_b,fun_b)
print("generating vtk")
N=100
for i in range(N+1):
    xyz=pfun_a(i*1/(N))
    meshio.write_points_cells('tetra.0.time.'+f'{i:04}'+'.vtk', xyz, {'tetra': F_a})
    writetet6('tetra.0.time.'+f'{i:04}'+'.tet6',xyz,xyz,F_a)
for i in range(N+1,2*N+2):
    xyz=pfun_b((i-100)*1/(N))
    meshio.write_points_cells('tetra.0.time.'+f'{i:04}'+'.vtk', xyz, {'tetra': F_b})
    writetet6('tetra.0.time.'+f'{i:04}'+'.tet6',xyz,xyz,F_b)
