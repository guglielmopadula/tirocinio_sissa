...
\headerbox{1a) Semi-discrete optimal transport}{name=introduction,below=auto,column=0,row=0,span=3}{
In the first line of research \textbf{semi-discrete optimal transport} is used to obtain an optimal transport map to find some intermediate geometries with some regularity constraints. An application is design optimization, in which the quantity of material for similar models must be the same. 
\\ \null\\}

\headerbox{1b) Generative models of deformated geometries}{name=introduction,aligned=introduction,column=3,row=0,span=3}{
 The second line consists in studying  \textbf{generative models} for shape optimization of complex geometries with a large number of parameters; the objective is also to reduce the number of relevant geometrical parameters, for example for modeling naval hulls, and creating new artificial geometries similar to real data, as there are non-generative techniques for creating new real geometries (see the package PyGem \cite{pygem} developed here at Sissa) but using them can be costly. 
}
...
\begin{posterbox}[name=vae,below=otm,span=6,column=0]{2b) Generative models for reduction in parameter space}
...
\node[inner sep=0pt,shift={(0,+1.65)}] (p21) [right=of decoder]{\includegraphics[scale=0.10]{mesh0}};
...
\node[squarednode2,rotate=-90,shift={(-1.65,0)}]      (decoder)       [right=of normal]{\textbf{Decoder}};
\node[inner sep=0pt,shift={(0,+1.65)}] (p21) [right=of decoder]{\includegraphics[scale=0.10]{mesh1}};
...
\item Generative adversarial networks: it is characterized by a generator that samples point cloud mesh of deformed Bulbous bow and by a discriminator that accepts real Bulbous (right figure)) and rejects deformed ones (left figure)).  \\
\null\\
\begin{tikzpicture}[thick,scale=1, every node/.style={scale=1},
...
\node[inner sep=0pt,shift={(0,+0.9	)}] (mesh)  [right=of generator]{\includegraphics[scale=0.10]{mesh1}};
